 ...
 * @param <C>
 */
public abstract class AbstractCoreAttrsContainerElement<T, C> extends AbstractContainerElement<T, C>
		implements CoreAttrs<T> {
	...
	public final T setClasses(final String tokens) {

		return this.setAttribute(Attribute.CLASS, new NMTokens(tokens));
	}


	/**
	 * Convenience Method to add a single class.
	 */
	public final T addClass(String className) {

		NMTokens classes = this.getClasses();
		if (classes == null) {
			classes = new NMTokens();
			this.setClasses(classes);
		}
		classes.addToken(className);
