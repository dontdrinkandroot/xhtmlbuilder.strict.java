 ...
 */
public class FrameTarget extends NMToken {

	private final String regex = "_(blank|self|parent|top)|[A-Za-z]\\c*";

	private final Pattern pattern = Pattern.compile(this.regex);


	public FrameTarget(final String token) {

		super(token);
		if (!this.pattern.matcher(token).matches()) {
			throw new IllegalArgumentException("Token must match: " + this.regex);
		}
	}
